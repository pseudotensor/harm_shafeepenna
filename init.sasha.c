...
int init_conservatives(FTYPE p[][N2M][N3M][NPR], FTYPE Utemp[][N2M][N3M][NPR], FTYPE U[][N2M][N3M][NPR])
{
  ...
  if(MAGTESTNOPOT( TESTNUMBER )){
    ...
    assign_fieldconservatives_pointvalues(U);
  }
...
int pi2Uavg_specific(FTYPE (*prim)[N2M][N3M][NPR], FTYPE (*Upoint)[N2M][N3M][NPR], FTYPE (*Uavg)[N2M][N3M][NPR])
{
  ...
  FTYPE dxdxp[NDIM][NDIM];
	...
	COMPZSLOOP( Uconsloop[FIS], Uconsloop[FIE], Uconsloop[FJS], Uconsloop[FJE], Uconsloop[FKS], Uconsloop[FKE] ){
	  //FULLLOOP {
    get_geometry(i, j, k, CENT, &geom);
    MYFUN(Utoprimgen(0,EVOLVEUTOPRIM, UEVOLVE, Uavg[i][j][k], &geom, p[i][j][k]),"init.c:pi2Uavg_specific()", "Utoprimgen", 1);
	}
...
int init_dsandvels(int *whichvel, int*whichcoord, int i, int j, int k, FTYPE *prim, FTYPE *pstag)
{
  ...
  *whichcoord=CARTMINKMETRIC;
	...
	prim[RHO] = 1.*(1. + randfact * (ranc(0) - 0.5));; //SASMARK guess the value for density, not given in L&W
	if( r < 0.2 ){
		prim[UU] = 5+ 25./2. * r * r;
...
#define CONSTECON1 1.0
  ...
  *whichcoord=CARTMINKMETRIC;
  ...
  prim[RHO] = rhocs0*(1. + randfact * (ranc(0) - 0.5));
...
#define FIELDLOOPBOOST 1 // 0,1,2
  ...
  *whichcoord=CARTMINKMETRIC;
  ...
  // choose wave
#if(1)
  PLOOP(whichpl) dU[whichpl]=dUslow[whichpl];
  mydU=dUslow;
  speed=speedslow;
#elif(0)
  ...
  PLOOP(whichpl) dU[whichpl]=dUalf[whichpl];
  mydU=dUalf;
  speed=speedalf;
#elif(0)
  PLOOP(whichpl) dU[whichpl]=dUfast[whichpl];
  mydU=dUfast;
  speed=speedfast;
#endif
  ...
  PLOOP(whichpl) delta[whichpl]=delta[RHO];

  // get U (conserved quantity) for this point
  PLOOP(whichpl) Uf[whichpl] = U0[whichpl] + dU[whichpl]*delta[whichpl];

  // set initial guess
  PLOOP(whichpl) prim[whichpl]=prim0[whichpl];
  ...
  PLOOP(whichpl) primtest[whichpl]=prim[whichpl];
  primtest[RHO] /= 1.0;
  ...
  PLOOP(whichpl) dualfprintf(fail_file,"pl=%d U=%21.15g %21.15g %21.15g\n",whichpl,Uf[whichpl],Ucheck[whichpl],Ureal[whichpl]);
  ...
  //	      ,0.5*primtest[RHO]*(primtest[U1]*primtest[U1]*geomreal.gcov[1][1] + primtest[U2]*primtest[U2]*geomreal.gcov[2][2] + primtest[U3]*primtest[U3]*geomreal.gcov[3][3])+primtest[UU]+0.5*(primtest[B1]*primtest[B1]*geomreal.gcov[1][1] + primtest[B2]*primtest[B2]*geomreal.gcov[2][2] + primtest[B3]*primtest[B3]*geomreal.gcov[3][3])
  //	      ,0.5*prim[RHO]*(prim[U1]*prim[U1]*geomreal.gcov[1][1] + prim[U2]*prim[U2]*geomreal.gcov[2][2] + prim[U3]*prim[U3]*geomreal.gcov[3][3])+prim[UU]+0.5*(prim[B1]*prim[B1]*geomreal.gcov[1][1] + prim[B2]*prim[B2]*geomreal.gcov[2][2] + prim[B3]*prim[B3]*geomreal.gcov[3][3])
  //	      ,prim[UU]);
#endif
...
int init_vpot2field_user(FTYPE A[][N1M+SHIFT1][N2M+SHIFT2][N3M+SHIFT3],FTYPE pr[][N2M][N3M][NPR])
{
  ...
  COMPFULLLOOP{
    // use analytical B3
    PLOOPBONLY(pl){
      // pstagscratch already overwritten, so restore
      if(pl==B3 && FLUXB==FLUXCTSTAG) pstagscratch[i][j][k][pl]=pr[i][j][k][pl];

      if(pl!=B3) pr[i][j][k][pl]=panalytic[i][j][k][pl];
    }
  }
...
int normalize_field(FTYPE p[][N2M][N3M][NPR])
{
  ...
  COMPZLOOP {
    get_geometry(i, j, k, CENT, &geom);
    MYFUN(get_state(p[i][j][k], &geom, &q) ,"init.c:normalize_field()", "get_state() dir=0", 1);
    mhd_calc_norestmass_em(p[i][j][k], TT, &geom, &q, mhdem);
