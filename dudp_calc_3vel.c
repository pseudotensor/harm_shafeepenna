...
int dudp_calc_3vel(int whichcons, FTYPE *pr, struct of_state *q, struct of_geom *geom,
	      FTYPE **alpha)
{
  FTYPE w, bsq;
  ...
  //	  for(j=1;j<=5;j++){
  //	    if(j==1) for(k=1;k<=5;k++) alpha[j][k] *= geom->g ;
  //	    else if((j==2)&&(NOGDET0==0)) for(k=1;k<=5;k++) alpha[j][k] *= geom->g ;
  //	    else if((j==3)&&(NOGDET1==0)) for(k=1;k<=5;k++) alpha[j][k] *= geom->g ;
  //	    else if((j==4)&&(NOGDET2==0)) for(k=1;k<=5;k++) alpha[j][k] *= geom->g ;
  //	    else if((j==5)&&(NOGDET3==0)) for(k=1;k<=5;k++) alpha[j][k] *= geom->g ;
  //	  }
  //	}
