...
// pother[]
FTYPE (*brface)[N2M][N3M];
FTYPE (*aphicorn)[N2M][N3M];
FTYPE Vpar;
FTYPE Omegastar;
FTYPE Bpole;
...
int init_primitives(FTYPE p[][N2M][N3M][NPR])
{
 ...
 COMPFULLLOOPP1{
  ...
  // copy over initial solution as analytic solution
  // SET ANALYTIC SOLUTION FROM vector potential-based solution
 COMPZSLOOP(-N1BND, N1-1+N1BND, -N2BND, N2-1+N2BND, -N3BND, N3-1+N3BND) PLOOP(pl){
...
int init_vpot(int l, int i, int j, int k, FTYPE *A)
{
  ...
  void set_ramesh_solution( int l, struct of_geom *geom, FTYPE *V, FTYPE (*prim)[N2M][N3M][NPR],FTYPE *A);
  int pp;
...
// assumes normal field in pr
int init_vpot_V(int l, struct of_geom *geom, FTYPE *V, FTYPE (*prim)[N2M][N3M][NPR],FTYPE *A)
{
  int get_analytic_vpot( int l, struct of_geom *geom, FTYPE *V, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *A );
...
void set_analytical_face(void)
{
  ...
  int get_analytic_omegaf( struct of_geom *geom, FTYPE *V, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *omegaf );
  FTYPE omegaf;
  ...
  FULLLOOP{
    ...
    l=3;
    init_vpot_V(l,&geom,V,p,&pother[APHICORN][i][j][k]);
    
    // assume we were setting A_\phi and not A_{x^3}, so need to correct
    // correct for 2\pi factor
    pother[APHICORN][i][j][k] *=dxdxp[PH][PH];
  ...
  FULLLOOP{
    ...
    l=3;
    init_vpot_V(l,&geom,V,p,&pother[APHICENT][i][j][k]);
    
    // assume we were setting A_\phi and not A_{x^3}, so need to correct
    // correct for 2\pi factor
    pother[APHICENT][i][j][k] *=dxdxp[PH][PH];
  ...
  FULLLOOP{
    ...
    pother[B1FACE1][i][j][k]=(Ah-Al)/(geom.g*(Xh[2]-Xl[2]));
  ...
  FULLLOOP{
    ...
    // use analytical definition of B^{x^1}
    pother[B1CENT][i][j][k]=(Ah-Al)/(geom.g*(Xh[2]-Xl[2]));
  ...
  FULLLOOP{
    ...
    pother[B1ZEUS][i][j][k]=(Ah-Al)/(geom.g*(Xh[2]-Xl[2]));
  ...
  FULLLOOP{
    ...
    pother[B1FLUXCTFACE1][i][j][k]=((Ahh+Alh)*0.5 - (Ahl+All)*0.5)/(geom.g*dx[2]);
  ...
  FULLLOOP{
    ...
    if(geom.g!=0.0){
      pother[B2FACE2][i][j][k]=-(Ah-Al)/(geom.g*(Xh[1]-Xl[1]));
    }
    else{
      pother[B2FACE2][i][j][k]=0.0;
    }
  ...
  FULLLOOP{
    ...
    // use analytical definition of B^{x^2}
    pother[B2CENT][i][j][k]=-(Ah-Al)/(geom.g*(Xh[1]-Xl[1]));
  ...
  FULLLOOP{
    ...
    if(geom.g!=0.0){
      pother[B2ZEUS][i][j][k]=-(Ah-Al)/(geom.g*(Xh[1]-Xl[1]));
    }
    else{
      pother[B2ZEUS][i][j][k]=0.0;
    }
  ...
  FULLLOOP{
    ...
    // in coordinate basis
    pother[OMEGAFFACE1][i][j][k]=mm*pow(B0*pow(Rin,nu),-1.0/nu)/dxdxp[3][3];
  ...
  FULLLOOP{
    ...
    // in coordinate basis
    pother[OMEGAFCENT][i][j][k]=omegaf / dxdxp[3][3];
  ...
  FULLLOOP{
    ...
    pother[RHOFACE1][i][j][k]=prtemp[RHO];
  ...
  FULLLOOP{
    ...
    if(pother[B1FACE1][i][j][k]>0){
      pother[VPARFACE1][i][j][k]=Vpar;
    }
    else{
      pother[VPARFACE1][i][j][k]=-Vpar;
    }
  ...
  FULLLOOP{ 
    ...
    pother[OMEGAFFACE2][i][j][k]= (mm/V[1]) / dxdxp[3][3];
    pother[B2FACE2SIMPLE][i][j][k] = -(B0*nu*pow(V[1],nu-2)) * (geom.gcov[2][2]);

    prtemp[RHO] = 0; //SASMARK
    //init_nodisk(FACE2, &whichvel, &whichcoord, i, j, k, prtemp);
      //    set_density_floors(&geom,prtemp,prtemp);
    pother[RHOFACE2][i][j][k]=prtemp[RHO];

    // assume Vpar in orthonormal basis
    pother[VPARFACE2][i][j][k]=Vpar;

    // not used
    pother[UUFACE2][i][j][k]=0.0;
...
int init_vpot2field(FTYPE A[][N1M+SHIFT1][N2M+SHIFT2][N3M+SHIFT3],FTYPE pr[][N2M][N3M][NPR])
{
  ...
  if(FLUXB==FLUXCTHLL){
    ...
    FULLLOOP{
      pr[i][j][k][B1]=pother[B1CENT][i][j][k];
      pr[i][j][k][B2]=pother[B2CENT][i][j][k];
      pr[i][j][k][B3]=0.0;
    }
  }
...
#if(USEMPI)
  MPI_Bcast(&nu,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&mm,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&ss,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&ucrit,1,MPI_FTYPE,0,MPI_COMM_GRMHD);

  MPI_Bcast(&Ttpow,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&jetalpha,1,MPI_FTYPE,0,MPI_COMM_GRMHD);


  MPI_Bcast(&NTHETA,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&theta,NTHETA,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&Thetavstheta,NTHETA,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&dThetadthetavstheta,NTHETA,MPI_FTYPE,0,MPI_COMM_GRMHD);
#endif
...
void set_ramesh_solution(int l, struct of_geom *geom, FTYPE *V, FTYPE (*prim)[N2M][N3M][NPR],FTYPE *A)
{
  ...
  if( ii < N1 + N1BND && jj < N2 + N2BND && kk < N3 + N3BND ) {  //SASMARKx: to avoid getting out of bounds
    //    prim[ii][jj][kk][RHO]=*A;
    prim[ii][jj][kk][B1]=0.0;
    prim[ii][jj][kk][B2]=0.0;
    prim[ii][jj][kk][B3]=0.0;
  } 
  ...
  //  get_analytic_omegaf( &geomc, Vc, prim, &omegafanalytic[ii][jj][kk][0] );
  //omegafanalytic[ii][jj][kk][0] /= dxdxp[3][3];

  // for testing purposes (dump999x), place omegafanalytic into internal energy.  This is set back to 0 before evolution.
  //  prim[ii][jj][kk][UU]=omegafanalytic[ii][jj][kk][0];
