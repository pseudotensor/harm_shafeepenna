...
int init_star(int *whichvel, int*whichcoord, int i, int j, int k, FTYPE *pr)
{
  ...
  // PLOOP(pl) dualfprintf(fail_file,"i=%d j=%d k=%d prstellar[%d]=%21.15g\n",i,j,k,pl,prstellar[pl]);
  ...
  PLOOP(pl) pratm[pl]=prstellar[pl]; // default value (this copies densities and fields)
  ...
  if(fabs(r)<4.0*MBH){
    ...
    przamobl[U1] = (geombl.gcon[0][1])/(geombl.gcon[0][0]) ;
    przamobl[U2] = (geombl.gcon[0][2])/(geombl.gcon[0][0]) ;
    przamobl[U3] = (geombl.gcon[0][3])/(geombl.gcon[0][0]) ;
    ...
    //PLOOP(pl) dualfprintf(fail_file,"i=%d j=%d k=%d prstellar[%d]=%21.15g pratm[%d]=%21.15g przamobl[%d]=%21.15g\n",i,j,k,pl,prstellar[pl],pl,pratm[pl],pl,przamobl[pl]);
  ...
  //  PLOOP(pl) dualfprintf(fail_file,"i=%d j=%d k=%d prstellar[%d]=%21.15g przamo=%21.15g pratmpost=%21.15g\n",i,j,k,pl,prstellar[pl],przamo[pl],pratm[pl]);
...
#if(USEMPI)
  MPI_Bcast(&NRADIAL,1,MPI_INT,0,MPI_COMM_GRMHD);

  MPI_Bcast(&MBH0,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&a0,1,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&QBH0,1,MPI_FTYPE,0,MPI_COMM_GRMHD);

  MPI_Bcast(&(radius[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(rho[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(ie[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(vr[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(omega3[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(yl[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(ynu[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(hcm[0]),NRADIAL,MPI_FTYPE,0,MPI_COMM_GRMHD);
#endif
