...
//
// below macros are similar to those farther below for divb, but no gdet stuff
#define absFgN(F,i,j,k,pl) (fabs(F[i][j][k][pl]))
...
#define Fgface(F,i,j,k,pl) (F[i][j][k][pl]*MYGDET(i,j,k,FACE1+pl-B1)) // assumes B1,B2,B3 in order for pl GODMARK
...
//
// below macros used for divb definition (with gdet)
#define absFg(F,i,j,k,pl) (fabs(F[i][j][k][pl]*MYGDET(i,j,k,CENT)))
#define absFgface(F,i,j,k,pl) (fabs(F[i][j][k][pl]*MYGDET(i,j,k,FACE1+pl-B1))) // assumes B1,B2,B3 in order for pl GODMARK
...
// PRMITIVE quantity
// FLUXCD divb lives at CENT
#define DIVBNORMFLUXCDPRIM(p,i,j,k) (MAX(MAX(dx[1],dx[2]),dx[3])/(THIRD*MYGDET(i,j,k,CENT)*fabs(\
