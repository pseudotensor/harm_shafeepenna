...
int bl2met2metp2v_genloc(int whichvel, int whichcoord, FTYPE *pr, int ii, int jj, int kk, int loc)
{
  ...
  gset_genloc(0,whichcoord,ii,jj,kk,loc,&geom);
  // convert whichvel-pr in whichcoord coords to ucon in whichcoord coordinates
  if (pr2ucon(whichvel,pr, &geom ,ucon) >= 1) FAILSTATEMENT("transforms.c:bl2met2metp2v_genloc()", "pr2ucon()", 1);
  ...
  // get prime geometry
  get_geometry(ii,jj,kk,loc,&geom) ;
...
int ucov_whichcoord2primecoords(int whichcoord, int ii, int jj, int kk, int loc, FTYPE *ucov)
{
  ...
  // get prime geometry
  get_geometry(ii,jj,kk,loc,&geomprime) ;

  // lower back
  lower_vec(ucon,&geomprime,ucov);
...
int bl2met2metp2v_gen(int whichvel, int whichcoord, int newwhichvel, int newwhichcoord, FTYPE *pr, int ii, int jj, int kk)
{
  ...
  // convert whichvel-pr in whichcoord coords to ucon in whichcoord coordinates
  if (pr2ucon(whichvel,pr, &geom ,ucon) >= 1) FAILSTATEMENT("transforms.c:bl2met2metp2v_gen()", "pr2ucon()", 1);
  ...
  // convert from MCOORD prime 4-vel to MCOORD prime WHICHVEL-vel(i.e. primitive velocity of evolution)
  ucon2pr(newwhichvel,ucon,&geom,pr);
...
int metp2met2bl(int whichvel, int whichcoord, FTYPE *pr, int ii, int jj, int kk)
{
  ...
  //  if (pr2ucon(WHICHVEL,pr, &geom ,ucon) >= 1) FAILSTATEMENT("transforms.c:metp2met2bl()", "pr2ucon()", 1);
  MYFUN(pr2ucon(WHICHVEL,pr, &geom ,ucon),"transforms.c:pr2ucon()","metp2met2bl",0);
