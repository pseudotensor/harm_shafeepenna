...
int Utoprim_coldgrmhd(FTYPE *U, struct of_geom *geom, FTYPE *pr, int *positivityproblem)
{
  ...
  FTYPE rho;
  int get_cold_pressure(FTYPE W, FTYPE Dsq, FTYPE gammasq, FTYPE *pressure);
  FTYPE pressure;
  ...
  *positivityproblem=0;
  ...
  SLOOPA(j) etacov[j]=0.0; // must be 0 for system to work
  ...
  get_cold_pressure(W, Dsq, gammasq, &pressure);
  // check if perssure >=0
  if(pressure<0.0){
  ...
  if(WHICHVEL!=VELREL4){
    ...
    ucon_calc_rel4vel_fromuconrel(uetacon,geom,ucon);
    ucon2pr(WHICHVEL,ucon,geom,pr);
  }

  // get code fields
  SLOOPA(j) pr[B1-1+j] = U[B1-1+j]; // no need to convert from Ueta since originally in U
...
int construct_Ueta(FTYPE *etacov, FTYPE *U, struct of_geom *geom, FTYPE *Ueta)
{
  ...
  DLOOP(j,k) projetauu[j][k] = geom->gcon[j][k] + etacon[j]*etacon[k];
...
int get_cold_pressure(FTYPE W, FTYPE Dsq, FTYPE gammasq, FTYPE *pressure)
{
  ...
  *pressure = pressure_W_vsq(W,sqrt(fabs(Dsq)),vsq);
...
void test_coldgrmhd_inversion(void)
{
 int i,j,k,pl;
  ...
  Utoprim_coldgrmhd(U,&geom,prout,&positivityproblem); // no need for initial guess since analytic inversion
