...
int mnewt(int ntrial, int mintrial, FTYPE x[], int n, FTYPE tolx, FTYPE tolf, FTYPE tolxallowed, FTYPE tolfallowed, FTYPE tolxreport, FTYPE tolfreport)
{
  int i = 0, j = 0, k = 0;
  ...
  if ((errf >= tolfreport)||(errx >= tolxreport)) {
    fprintf(stderr,"Couldn't find solution: %d %d %g %g\n",icurr,jcurr,errf,errx);
  }
