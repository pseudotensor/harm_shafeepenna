...
static int check_on_inversion(PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr, struct of_geom *ptrgeom, FTYPE *Uold, FTYPE *Unew);
static int debug_utoprimgen(PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr, struct of_geom *ptrgeom, FTYPE *Uold, FTYPE *Unew);
static int compare_ffde_inversions(PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr, struct of_geom *ptrgeom, FTYPE *Ugeomfree0, FTYPE*Ugeomfree, FTYPE *Uold, FTYPE *Unew);


int Utoprimgen(int finalstep, int evolvetype, int inputtype,FTYPE *U,  struct of_geom *ptrgeom, FTYPE *pr)
{
  // debug
  int i, j, k;
  ...
  int Utoprimgen_pick(int which, int parameter, FTYPE *Ugeomfree, struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr);
  int Utoprimgen_compare(int parameter, FTYPE *Ugeomfree, struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr);
  int Utoprimgen_tryagain(int parameter, FTYPE *Ugeomfree0, FTYPE *Ugeomfree,struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr);
  int Utoprimgen_tryagain2(int parameter, FTYPE *Ugeomfree0, FTYPE *Ugeomfree,struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr);
  PFTYPE lpflag,hotpflag,coldpflag;
  FTYPE prhot[NPR],prcold[NPR];
  int trycoldinversion(PFTYPE hotpflag, FTYPE *pr0, FTYPE *pr, FTYPE *Ugeomfree, FTYPE *Ugeomfree0, struct of_geom *ptrgeom);
  ...
  if(EOMTYPE==EOMGRMHD){
    ...
    if(DOENTROPY!=DOEVOLVEDIRECTENTROPY){
      ...
      Utoprimgen_tryagain2(EVOLVENOENTROPY, Ugeomfree0, Ugeomfree, ptrgeom, &pflag[ptrgeom->i][ptrgeom->j][ptrgeom->k][FLAGUTOPRIMFAIL], pr0, pr);
#endif
    }
    ...
    if(0){ // Jon's old inversion
      ...
      compare_ffde_inversions(&lpflag, pr0, pr, ptrgeom, Ugeomfree0, Ugeomfree, Uold, Unew);
    }
...
#if(EOMTYPE == EOMGRMHD)
  if(finalstep){
    negdensitycheck(pr, &pflag[ptrgeom->i][ptrgeom->j][ptrgeom->k][FLAGUTOPRIMFAIL]);
  }
#endif
  ...
  //
  ////////////////////////////////////////
  check_on_inversion(&lpflag, pr0, pr, ptrgeom, Uold, Unew);
#endif
...
static int check_on_inversion(PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr, struct of_geom *ptrgeom, FTYPE *Uold, FTYPE *Unew)
{
  ...
  if(!(*lpflag)){ // only report if not failure
    ...
    PLOOP(k){
      ...
      //      Unormalnew[k]*=ptrgeom->g;
      //      Unormalold[k]*=ptrgeom->g;
...
static int negdensitycheck(FTYPE *prim, PFTYPE *pflag)
{
  ...
  if( *pflag == UTOPRIMNOFAIL  || *pflag == UTOPRIMFAILUNEG ||  *pflag == UTOPRIMFAILRHONEG ) {
...
int Utoprimdiss(int evolvetype, int inputtype,FTYPE *U,  struct of_geom *ptrgeom, FTYPE *pr, PFTYPE *otherpflag)
{
  // debug
  int i, j, k;
  FTYPE Ugeomfree[NPR],Ugeomfree0[NPR];
  ...
  *otherpflag=UTOPRIMNOFAIL;
  ...
  MYFUN(Utoprim(whichentropy, Uold, ptrgeom, otherpflag, pr),"step_ch.c:Utoprimgen()", "Utoprim", 1);

  // DEBUG:
  //  PLOOP(pl) dualfprintf(fail_file,"otherpflag=%d Uold[%d]=%21.15g pr0[%d]=%21.15g pr[%d]=%21.15g\n",*otherpflag,pl,Uold[pl],pl,pr0[pl], pl,pr[pl]);
...
int Utoprimgen_tryagain(int parameter, FTYPE *Ugeomfree0, FTYPE *Ugeomfree,struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr)
{
  int Utoprimgen_tryagain_substep(int which, int parameter, FTYPE *Ugeomfree0, FTYPE*Ugeomfree, struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr);
...
int Utoprimgen_tryagain2(int parameter, FTYPE *Ugeomfree0, FTYPE *Ugeomfree,struct of_geom *ptrgeom, PFTYPE *lpflag, FTYPE *pr0, FTYPE *pr)
{
  ...
  Utoprimgen_tryagain_substep(UTOPRIM2DFINAL, parameter, Uorig0, Uorig, ptrgeom, lpflag, pr0, pr);
...
int Utoprimloop(FTYPE unew[][N2M][N3M][NPR],FTYPE pf[][N2M][N3M][NPR])
{
  ...
  ZLOOP{
    ...
    // invert True U->p
    MYFUN(Utoprimgen(0,EVOLVEUTOPRIM,UEVOLVE,unew[i][j][k], &geom, pf[i][j][k]),"step_ch.c:advance()", "Utoprimgen", 1);
  }
  return(0);
}
...
void filterffde(int i, int j, int k, FTYPE *pr)
{
  ...
  Utoprimgen(0,EVOLVEUTOPRIM,UNOTHING,U,&geom,prout);
