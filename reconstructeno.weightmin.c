...
static void interpolate_diffweighted_weights(int preforder, int whichquantity, int dir, FTYPE (*ystencilvar)[2][NBIGM], FTYPE (*normalystencilvar)[NBIGM], weno_weights_t (*stencil_weights_array_pl)[NBIGM]);
...
static weno_weights_t a_stencil_weights_array_other[NBIGM];  //for storing the weights for a one-dimensional array of values; used by stencil reduction
...
static weno_weights_t a_stencil_weights_array_storemin[NBIGM];
weno_weights_t *stencil_weights_array_storemin = &(a_stencil_weights_array_storemin[NBIGBND]);

static weno_weights_t a_stencil_weights_array_pl[NPR][NBIGM];
weno_weights_t (*stencil_weights_array_pl)[NBIGM];

static FTYPE a_normalystencilvar[NPR][NBIGM];
...
void compute_multipl_weno(int MULTIPLTYPE, int whichquantity, int dir, int do_weight_or_recon, int recontype, int whichreduce, int preforder, int bs, int ps, int pe, int be, int *minorder, int *maxorder, int *shift,   FTYPE *shockindicator, FTYPE *stiffindicator, FTYPE *Vline,  FTYPE *Pline, FTYPE (*df)[NUMDFS][NBIGM], FTYPE (*dP)[NBIGM], FTYPE (*etai)[NBIGM], FTYPE (*monoindicator)[NUMMONOINDICATORS][NBIGM], FTYPE (*yprim)[2][NBIGM], FTYPE (*ystencilvar)[2][NBIGM], FTYPE (*yin)[2][NBIGM], FTYPE (*yout)[2][NBIGM], FTYPE (*youtpolycoef)[MAXSPACEORDER][NBIGM])
{
  // for NUMPRIMLOOP:
  ...
  if( MULTIPLTYPE == MINIMIZE_ALL_WEIGHTS ) { //a2c on conserved
    ...
    NUMPRIMLOOP(pl){
      pass_1d_line_weno_withweights( whichquantity, dir, WEIGHT_CALC, stencil_weights_array_allpl[pl], recontype, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, stiffindicator, Vline, Pline, df[pl], dP, etai[pl], monoindicator[pl], yprim, ystencilvar[pl], yin[pl], yout[pl],youtpolycoef[pl]);
    ...
    NUMPRIMLOOP(pl) {
      ...
      pass_1d_line_weno_withweights( whichquantity, dir, RECON_CALC, stencil_weights_array_allpl[pl], recontype, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, stiffindicator, Vline, Pline, df[pl], dP, etai[pl], monoindicator[pl], yprim, ystencilvar[pl], yin[pl], yout[pl], youtpolycoef[pl]);
    }
	...
	pass_1d_line_weno_withweights( whichquantity, dir, WEIGHT_CALC, stencil_weights_array_pl[pl], recontype, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, stiffindicator, Vline, Pline, df[pl], dP, etai[pl], monoindicator[pl], yprim, ystencilvar[pl], yin[pl], yout[pl], youtpolycoef[pl]);
    ...
    // CHANGINGMARK:
    interpolate_diffweighted_weights(preforder, whichquantity, dir, ystencilvar, normalystencilvar, stencil_weights_array_pl);
    ...
    NUMPRIMLOOP(pl) {
      ...
      pass_1d_line_weno_withweights( whichquantity, dir, RECON_CALC, stencil_weights_array_pl[pl], recontype, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, stiffindicator, Vline, Pline, df[pl], dP, etai[pl], monoindicator[pl], yprim, ystencilvar[pl], yin[pl], yout[pl], youtpolycoef[pl]);
    }
...
static void interpolate_diffweighted_weights(int preforder, int whichquantity, int dir, FTYPE (*ystencilvar)[2][NBIGM], FTYPE (*normalystencilvar)[NBIGM], weno_weights_t (*stencil_weights_array_pl)[NBIGM])
{
  ...
  NUMPRIMLOOPGEN(pllocal,pl){
	...
	myterm = 0.0;
	dFsum = 0.0;
	NUMPRIMLOOPGEN(pllocalother,plother){
