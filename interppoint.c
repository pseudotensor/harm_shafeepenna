...
void slope_lim_pointtype(int interporflux, int realisinterp, int pl, int dir, int idel, int jdel, int kdel, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*p2interp)[N2M][N3M][NPR2INTERP], FTYPE (*dq)[N2M][N3M][NPR2INTERP], FTYPE (*pleft)[N2M][N3M][NPR2INTERP], FTYPE (*pright)[N2M][N3M][NPR2INTERP])
{
  int i,j,k;
  void slope_lim_point(int realisinterp, int reallim, int pl, int startorderi, int endorderi, FTYPE *yreal, FTYPE*y, FTYPE *dq,FTYPE *left,FTYPE *right);
  void slope_lim_point_allpl(int realisinterp, int dir, int reallim, int startorderi, int endorderi, FTYPE **yreal, FTYPE **y, FTYPE *dq,FTYPE *left,FTYPE *right);
  ...
  if(reallim==PARAFLAT || reallim == MCSTEEP){
    ...
    COMPZSLOOP( is, ie, js, je, ks, ke ){
      ...
      PLOOPINTERP(plpl) for(l=startorderi;l<=endorderi;l++){
	...
	PALLREALLOOP(plpl) for(l=startorderi;l<=endorderi;l++){
	  yrealpl[plpl][l]=primreal[i + l*idel][j + l*jdel][k + l*kdel][plpl];
	}
      }

      icurr=i; jcurr=j; kcurr=k;
      slope_lim_point_allpl(realisinterp, dir,reallim,startorderi,endorderi,yrealpl,ypl,
			    &dq[i][j][k][0],&pleft[i][j][k][0],&pright[i][j][k][0]
			    );
    }// end over loop
  } // end if doing all pl at once
  else{
   ...
   COMPZSLOOP( is, ie, js, je, ks, ke ) {
	...
	for(l=startorderi;l<=endorderi;l++){
	  yreal[l]=primreal[i + l*idel][j + l*jdel][k + l*kdel][pl];
	}
      }
      
      icurr=i; jcurr=j; kcurr=k;
      slope_lim_point(realisinterp, reallim,pl,startorderi,endorderi,yreal,y,
...
void slope_lim_point_allpl(int realisinterp, int dir,int reallim, int startorderi, int endorderi, FTYPE **yreal, FTYPE **y, FTYPE *dq,FTYPE *left,FTYPE *right)
{
  ...
  void mcsteeppl(int realisinterp, int dir, FTYPE **yreal, FTYPE **y, FTYPE *lout, FTYPE *rout);

  switch(reallim){
  case PARAFLAT:
    parapl(realisinterp, dir,yreal,y,left,right);
    break;
  case MCSTEEP:
    mcsteeppl(realisinterp, dir,yreal,y,left,right);
    break;
  default:
...
void mcsteeppl(int realisinterp, int dir, FTYPE **yrealpl, FTYPE **ypl, FTYPE *loutpl, FTYPE *routpl)
{
  ...
  PLOOPINTERP(pl){
    ...
    checkparamonotonicity(dqrange, pl, ypl[pl], ddq, dqpl[pl], &loutpl[pl], &routpl[pl], &loutpl[pl], &routpl[pl]);
#endif
