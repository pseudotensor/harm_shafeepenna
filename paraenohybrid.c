...
#define MIXEND 0.4
  ...
  if(FULLHYBRID==1 && !allpara || FULLHYBRID==0 || PARAMODWENO==0){
    returnedvalue=eno_line_c2e( whichquantity, dir, do_weight_or_recon, stencil_weights_array, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator,  stiffindicator, V, P, df, dP, etai, monoindicator, Pindicator, yin, yout_left, yout_right, youtpolycoef ); 
  }
  ...
  if(PARAMODWENO && MAXBND>=4){ // then assume can store enough dq's for PARA-like steepen/flat/monocheck
    ...
    if(FULLHYBRID==1 && !allweno || FULLHYBRID==0){
      paraprocess_line_c2e( whichquantity, dir, do_weight_or_recon, stencil_weights_array, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit,  shockindicator, stiffindicator, parafrac, V, P, df, dP, etai, monoindicator, Pindicator, yin, yout_left, yout_right );  
    }
  }
