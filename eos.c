...
FTYPE compute_qdot_coldgrmhd(FTYPE rho0, FTYPE u)
{
  return(0.0);
}

int compute_sources_EOS_coldgrmhd(FTYPE *pr, struct of_geom *geom, struct of_state *q, FTYPE *Ui, FTYPE *dUother, FTYPE(*dUcomp)[NPR])
{
  return(0);
}


void compute_allextras_coldgrmhd(int justnum, FTYPE rho0, FTYPE u, int *numextrasreturn, FTYPE *extras)
{
  return;
}

int get_extrasprocessed_coldgrmhd(int doall, int i, int j, int k, FTYPE *pr, FTYPE *extras, FTYPE *processed)
{
  return(0);
}


FTYPE compute_temp_coldgrmhd(FTYPE rho0, FTYPE u)
{
  return(0.0);
}

void compute_EOS_parms_coldgrmhd(FTYPE (*prim)[N2M][N3M][NPR])
{
  return; // do nothing
}

void store_EOS_parms_coldgrmhd(int i, int j, int k, int numparms, FTYPE *parlist)
{
  return; // do nothing
}
void get_EOS_parms_coldgrmhd(int i, int j, int k, int*numparms, FTYPE *parlist)
{
  return; // do nothing
}
...
// radiation rate
FTYPE compute_qdot(FTYPE rho0, FTYPE u)
{
  return( (*ptr_compute_qdot) (rho0,u) );
}


int compute_sources_EOS(FTYPE *pr, struct of_geom *geom, struct of_state *q, FTYPE *Ui, FTYPE *dUother, FTYPE(*dUcomp)[NPR])
{
  return( (*ptr_compute_sources_EOS) (pr, geom, q, Ui, dUother, dUcomp));
}



void compute_allextras(int justnum, FTYPE rho0, FTYPE u, int *numextrasreturn, FTYPE *extras)
{
  (*ptr_compute_allextras) (justnum,rho0,u,numextrasreturn,extras);
  return;
}

int get_extrasprocessed(int doall, int i, int j, int k, FTYPE *pr, FTYPE *extras, FTYPE *processed)
{
  return((*ptr_get_extrasprocessed) (doall,i,j,k,pr,extras,processed));
}



FTYPE compute_temp(FTYPE rho0, FTYPE u)
{
  return( (*ptr_compute_temp) (rho0,u) );
}


void compute_EOS_parms(FTYPE (*prim)[N2M][N3M][NPR])
{
  (*ptr_compute_EOS_parms) (prim);
}

void store_EOS_parms(int i, int j, int k, int numparms, FTYPE *parlist)
{
  (*ptr_store_EOS_parms) (i,j,k,numparms,parlist);
}
void get_EOS_parms(int i, int j, int k, int*numparms, FTYPE *parlist)
{
  (*ptr_get_EOS_parms) (i,j,k,numparms, parlist);
}
