...
void (*nrfuncv)(int n, FTYPE v[], FTYPE f[]);
...
void newt(FTYPE x[], int n, int *check,
	void (*vecfunc)(int, FTYPE [], FTYPE []))
{
	void fdjac(int n, FTYPE x[], FTYPE fvec[], FTYPE **df,
		void (*vecfunc)(int, FTYPE [], FTYPE []));
	...
	for (its=1;its<=MAXITS;its++) {
	  ...
	  /*
		...
		*/		
		...
		lnsrch(n,xold,fold,g,p,x,&f,stpmax,check,nrfmin);
		test=0.0;
		for (i=1;i<=n;i++)
			if (fabs(fvec[i]) > test) test=fabs(fvec[i]);
		if (test < TOLF) {
