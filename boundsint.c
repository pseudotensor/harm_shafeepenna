...
int bound_pflag_user(int boundstage, SFTYPE boundtime, int boundvartype, PFTYPE prim[][N2M][N3M][NUMPFLAGS])
{
  int i, j, k, pl;
  int failreturn;
  ...
  if ( (mycpupos[1] == 0)&&(mycpupos[1] == ncpux1 - 1) ) {
	...
	rj=j;
	rk=k;
	LOOPBOUND1IN FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri+1+i][rj][rk][pl];

	// copy from lower side to upper boundary zones
	ri=riin;
	rj=j;
	rk=k;
	LOOPBOUND1OUT FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri+(i-N1)][rj][rk][pl];
      }
    }
  }
  ...
  if ( (mycpupos[2] == 0)&&(mycpupos[2] == ncpux2 - 1) ) {
	...
	rj=rjout;
	rk=k;
	LOOPBOUND2IN FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj+1+j][rk][pl];

	// copy from lower side to upper boundary zones
	ri=i;
	rj=rjin;
	rk=k;
	LOOPBOUND2OUT FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj+(j-N2)][rk][pl];
      }
    }
  }
  ...
  if ( (mycpupos[3] == 0)&&(mycpupos[3] == ncpux3 - 1) ) {
	...
	rj=j;
	rk=rkout;
	LOOPBOUND3IN FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk+1+k][pl];

	// copy from lower side to upper boundary zones
	ri=i;
	rj=j;
	rk=rkin;
	LOOPBOUND3OUT FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk+(k-N3)][pl];
      }
    }
  }
  ...
  if (mycpupos[1] == 0) {
	...
	rj=j;
	rk=k;
	LOOPBOUND1IN FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[1] == ncpux1 - 1) {
	...
	rj=j;
	rk=k;
	LOOPBOUND1OUT FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[2] == 0) {
	...
	rj=rjin;
	rk=k;
	LOOPBOUND2IN FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[2] == ncpux2 - 1) {
	...
	rj=rjout;
	rk=k;
	LOOPBOUND2OUT FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[3] == 0) {
	...
	rj=j;
	rk=rkin;
	LOOPBOUND3IN FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[3] == ncpux3 - 1) {
	...
	rj=j;
	rk=rkout;
	LOOPBOUND3OUT FBOUNDLOOP(pl) prim[i][j][k][pl] = prim[ri][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[1] == 0) {
	...
	rk=k;
	// symmetric copy
	LOOPBOUND1IN FBOUNDLOOP(pl)  prim[i][j][k][pl] = prim[ri+(ri-i-1)][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[1] == ncpux1 - 1) {
	...
	rj=j;
	rk=k;
	LOOPBOUND1OUT FBOUNDLOOP(pl)  prim[i][j][k][pl] = prim[ri+(ri-i+1)][rj][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[2] == 0) {
	...
	rk=k;
	// symmetric copy
	LOOPBOUND2IN FBOUNDLOOP(pl)  prim[i][j][k][pl] = prim[ri][rj+(rj-j-1)][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[2] == ncpux2 - 1) {
	...
	rj=rjout;
	rk=k;
	LOOPBOUND2OUT FBOUNDLOOP(pl)  prim[i][j][k][pl] = prim[ri][rj+(rj-j+1)][rk][pl];
      }
    }
  }
  ...
  if (mycpupos[3] == 0) {
	...
	rk=rkin;
	// symmetric copy
	LOOPBOUND3IN FBOUNDLOOP(pl)  prim[i][j][k][pl] = prim[ri][rj][rk+(rk-k-1)][pl];
      }
    }
  }
  ...
  if (mycpupos[3] == ncpux3 - 1) {
	...
	rj=j;
	rk=rkout;
	LOOPBOUND3OUT FBOUNDLOOP(pl)  prim[i][j][k][pl] = prim[ri][rj][rk+(rk-k+1)][pl];
      }
    }
  }
