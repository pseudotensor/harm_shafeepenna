...
int vectorpot_useflux(int stage, FTYPE pr[][N2M][N3M][NPR])
{
  int flux_point2avg(int stage, int whichmaem, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR], FTYPE (*fluxvecother[NDIM])[N2M][N3M][NPR]);
  int Nvec[NDIM];
...
int flux_point2avg(int stage, int whichmaem, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR], FTYPE (*fluxvecother[NDIM])[N2M][N3M][NPR])
{
  int flux_deaverage_fluxrecon(int stage, int whichmaem, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR], FTYPE (*fluxvecother[NDIM])[N2M][N3M][NPR]);
  int bound_flux_fluxrecon(int stage, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
  int debug_boundfluxinitial(FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
  int debug_boundfluxfinal(FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);

  int flux_integrate_fluxsplit(int stage, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);

  int flux_integrate_finitevolume(int stage, int whichmaem, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
...
int flux_deaverage_fluxrecon(int stage, int whichmaem, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR], FTYPE (*fluxvecother[NDIM])[N2M][N3M][NPR])
{
  ...
  int simple_a2c_limit_onepl(int dotransverse, int dir, int pl, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*in)[N2M][N3M][NPR], FTYPE (*out)[N2M][N3M][NPR]);
  ...
  int ftemp;
  int emforvectorpot_fluxrecon(int stage, int isemf, int *fluxdirlist, int *pldirlist, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
  ...
  int extralimiting;
  int weightsplittype;
  FTYPE (*stencilvar)[N2M][N3M][NPR];
  FTYPE (*otherin)[N2M][N3M][NPR];
...
#if(0)
  LOOPF2 dualfprintf(fail_file,"BEFORE: emf(0)=%21.15g %21.15g emf(N1)=%21.15g %21.15g diff0-N1 = %21.15g\n",fluxvec[1][0][j][0][B2],-fluxvec[2][0][j][0][B1],fluxvec[1][N1][j][0][B2],-fluxvec[2][N1][j][0][B1],fluxvec[1][0][j][0][B2]-fluxvec[1][N1][j][0][B2]);
#endif
  ...
  PLOOP(pl) dotranslocal[pl]=do_transverse_flux_integration[pl];
  ...
  // any trans
  dotrans=0;
  PLOOP(pl) dotrans+=dotranslocal[pl];
  ...
  if(dotrans){
	...
	if(nprlocalend>-1){
	  ...
	  /////////////
	  if(extralimiting){
	    COMPFULLLOOP PLOOP(pl) fluxvectemp[i][j][k][pl] = fluxvec[dir][i][j][k][pl];
	  }
	  ...
	  if(fluxvecother==NULL){
	    COMPFULLLOOP PLOOP(pl) stencilvartemp[i][j][k][pl] = fluxvec[dir][i][j][k][pl];
	  }
	    ...
	    if(whichmaem==ISMAANDEM || (whichmaem==ISMAONLY && splitmaem==0) || whichmaem==ISEMONLY){
	      COMPFULLLOOP PLOOP(pl) stencilvartemp[i][j][k][pl] = fluxvec[dir][i][j][k][pl] + fluxvecother[dir][i][j][k][pl];
	    }
	      ...
	      COMPFULLLOOP{
		PLOOPNOB1(pl)  stencilvartemp[i][j][k][pl] = fluxvec[dir][i][j][k][pl] + fluxvecother[dir][i][j][k][pl];
		PLOOPNOB2(pl)  stencilvartemp[i][j][k][pl] = fluxvec[dir][i][j][k][pl] + fluxvecother[dir][i][j][k][pl];
		...
		pl=UU+dir; stencilvartemp[i][j][k][pl] += fluxvec[dir][i][j][k][FLUXSPLITPMA(dir)];
		...
		pl=FLUXSPLITPMA(dir); stencilvartemp[i][j][k][pl] = fluxvec[dir][i][j][k][pl] + fluxvec[dir][i][j][k][UU+dir] + fluxvecother[dir][i][j][k][UU+dir];
		...
		// assume pdiag can't be mixed with EM term, but for pressure term add back in ram pressure term
		// CHANGINGMARK:
		pl=FLUXSPLITPMA(dir); stencilvartemp[i][j][k][pl] += fluxvec[dir][i][j][k][UU+dir];
		pl=UU+dir; stencilvartemp[i][j][k][pl] += fluxvec[dir][i][j][k][FLUXSPLITPMA(dir)];
#endif
#if(0)
		// CHANGINGMARK:
		pl=UU+dir;  fluxvec[dir][i][j][k][pl] += fluxvec[dir][i][j][k][FLUXSPLITPMA(dir)];
		pl=FLUXSPLITPMA(dir); fluxvec[dir][i][j][k][pl] =0.0;
#endif
		// END  DEBUG:
		/////////////////////
#endif
 	    ...
 	    if(weightsplittype==MASSENERGYMOMENTUM_IS_COUPLED_WEIGHTS){
	      ...
	      ///////////////
	      COMPFULLLOOP{
		get_geometry(i,j,k,FACE1+dir-1,&geom); // location of flux is always on face
		...
		for(odirindex=0;odirindex<=1;odirindex++){
		  ...
		  pl=UU+theotherdir;
		  stencilvartemp[i][j][k][pl] *= sqrt(fabs(geom.gcon[theotherdir][theotherdir])/(fabs(geom.gcon[dir][dir])+SMALL));
		}
		...
		PLOOP(pl){
		  if(pl==RHO || (pl>=B3+1 && pl<NPR)){
		    ...
		    stencilvartemp[i][j][k][pl] = fabs(fluxvec[dir][i][j][k][UU+dir])*stencilvartemp[i][j][k][pl]/(fabs(fluxvec[dir][i][j][k][RHO])+SMALL);
		  }
		}
	      }// end COMPFULLLOOP
	  ...
	  if(fluxvecother!=NULL || weightsplittype==MASSENERGYMOMENTUM_IS_COUPLED_WEIGHTS){
	    stencilvar=stencilvartemp; // use modified specialized stencil variable
	  }
	  ...
	  if(extralimiting){
	    ...
	    PLOOP(pl) simple_a2c_limit_onepl(0, dir, pl, pr, fluxvectemp, fluxvec[dir]);
	  }
...
//
// convert quasi-averaged fluxes to quasi-point fluxes
int field_integrate_fluxrecon(int stage, FTYPE pr[][N2M][N3M][NPR], FTYPE quasifield[][N2M][N3M][NPR], FTYPE pointfield[][N2M][N3M][NPR])
{
  int odir1,odir2;
  int dir;
  int idel, jdel, kdel, face, idel1, idel2, jdel1, jdel2, kdel1, kdel2;
  ...
  DIMENLOOP(dir){
    ...
    if(dodir[dir]==0 ){
      ...
      COMPFULLLOOP pointfield[i][j][k][pldir]=quasifield[i][j][k][pldir];
    }
  }
...
int initial_averageu_fluxrecon(int *fieldfrompotential, FTYPE prim[][N2M][N3M][NPR], FTYPE Upoint[][N2M][N3M][NPR], FTYPE Uavg[][N2M][N3M][NPR])
{
  int field_deaverage_fluxrecon(int *fieldfrompotential, FTYPE pr[][N2M][N3M][NPR], FTYPE pointfield[][N2M][N3M][NPR], FTYPE quasifield[][N2M][N3M][NPR]);
  int i,j,k,pl;
  ...
  if(extrazones4emf) field_deaverage_fluxrecon(fieldfrompotential, prim, Upoint, Uavg);
    ...
    PLOOPBONLY(pl) Uavg[i][j][k][pl]=Upoint[i][j][k][pl];
  }

  //  FULLLOOP PLOOP(pl){
  //    dualfprintf(fail_file,"Uavg[%d][%d][%d][%d]=%21.15g\n",i,j,k,pl,Uavg[i][j][k][pl]);
  //  }
...
int field_deaverage_fluxrecon(int *fieldfrompotential, FTYPE pr[][N2M][N3M][NPR], FTYPE pointfield[][N2M][N3M][NPR], FTYPE quasifield[][N2M][N3M][NPR])
{
	...
	if(nprlocalend>-1){
	  ...
	  if(extralimiting){
	    if(pointfield==quasifield){
	      ...
	      COMPFULLLOOP intemp[i][j][k][pldir] = pointfield[i][j][k][pldir];
	    }
  ...
  DIMENLOOP(dir){
	...
	COMPFULLLOOP quasifield[i][j][k][pldir]=pointfield[i][j][k][pldir];
      }
    }
  }
...
int field_Bhat_fluxrecon(FTYPE pr[][N2M][N3M][NPR], FTYPE pointfield[][N2M][N3M][NPR], FTYPE quasifield[][N2M][N3M][NPR])
{
  ...
  if(dotransem){
	...
	if(Nvec[fluxdir]>1){
	  ...
	  if(nprlocalend>-1){
	    ...
	    if(extralimiting){
	      COMPFULLLOOP fluxvectemp[i][j][k][plforflux] = fluxvec[fluxdir][i][j][k][plforflux];
	    }
	    ...
	    if(extralimiting){
	      simple_a2c_limit_onepl(1,fluxdir, plforflux, pr, fluxvectemp, fluxvec[fluxdir]);
	    }
	  ...
	  if(isemf) if(Nvec[fluxdir]!=1 && Nvec[pldir]!=1) COMPFULLLOOP fluxvec[pldir][i][j][k][B1+fluxdir-1] = - fluxvec[fluxdir][i][j][k][B1+pldir-1];
...
int debug_boundfluxinitial(FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR])
{
  ...
  if(nstep==30){
    ...
    FULLLOOP{
      ...
      PLOOP(pl) fprintf(fail_file,"%g %g ",fluxvec[1][i][j][k][pl],fluxvec[2][i][j][k][pl]);
      fprintf(fail_file,"\n");
    }
    //		myexit(0);
  }
...
int debug_boundfluxfinal(FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR])
{
  ...
  if(nstep==30){
    ...
    FULLLOOP{
      ...
      PLOOP(pl) fprintf(log_file,"%g %g ",fluxvec[1][i][j][k][pl],fluxvec[2][i][j][k][pl]);
      fprintf(log_file,"\n");
    }
    myexit(0);
  }
...
int bound_flux_fluxrecon(int stage, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR])
{
  ...
  if(DOENOFLUX==ENOFLUXRECON && BOUNDFLUXRECON==1){
    ...
    bound_flux(STAGEM1,t,BOUNDFLUXTYPE,fluxvec[1],fluxvec[2],fluxvec[3]);
...
int vectorpot_fluxreconorfvavg(int stage, FTYPE pr[][N2M][N3M][NPR], FTYPE (*A)[N1M+SHIFT1][N2M+SHIFT2][N3M+SHIFT3])
{
  int emforvectorpot_fluxrecon(int stage, int isemf, int *fluxdirlist, int *pldirlist, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
  int emforvectorpot_fvavg(int stage, int isemf, int *fluxdir, int *pldir, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
  ...
  if(dotransem){
	...
	COMPFULLLOOP fluxvec[fluxdir][i][j][k][plforflux]=signforflux*A[dir][i][j][k];
      }
    ...
    DIMENLOOP(dir){
      ...
      if(fluxdir!=0 && Nvec[fluxdir]>1) COMPFULLLOOP A[dir][i][j][k]=signforflux*fluxvec[fluxdir][i][j][k][plforflux];
...
int flux_integrate_finitevolume(int stage, int whichmaem, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR])
{
  ...
  int whichpltoavg[NPR];
  int ifnotavgthencopy[NPR];
  int emforvectorpot_fvavg(int stage, int isemf, int *fluxdirlist, int *pldirlist, FTYPE pr[][N2M][N3M][NPR], int *Nvec, FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR]);
  ...
  // any trans
  dotrans=0;
  PLOOP(pl) dotrans+=do_transverse_flux_integration[pl];
  ...
  if(dotransem){
	...
	if(nprlocalend>-1){
	  ...
	  if(isemf){
		...
		//	    COMPFULLLOOP fluxvec[fluxdir][i][j][k][plforflux] -= (
	      }
	    }
	  }
	  ...
	  if(isemf){
	    if(extralimiting){
	      ...
	      limit_fluxc2a_prim_change( fluxdir, pr, fluxvectemp, fluxvec[fluxdir] );
	    }
	  }
...
int deaverage_fields_fv(FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*in)[N2M][N3M][NPR], FTYPE (*out)[N2M][N3M][NPR])
{
  int whichpltoavg[NPR];
  int ifnotavgthencopy[NPR];
  int doconsem;
  ...
  int simple_a2c_limit_field(FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*in)[N2M][N3M][NPR], FTYPE (*out)[N2M][N3M][NPR]);
  int pl;
  int i,j,k;
  ...
  if(doconsem){
    ...
    COMPFULLLOOP PLOOP(pl) out[i][j][k][pl]=in[i][j][k][pl];
...
int simple_a2c_limit_onepl(int dotransverse, int dir, int pl, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*in)[N2M][N3M][NPR], FTYPE (*out)[N2M][N3M][NPR])
{
  ...
  COMPFULLLOOP{
    ...
    out[i][j][k][pl] = yfrac*out[i][j][k][pl] + (1.0-yfrac)*myin;
...
int initial_averageu_fv(int *fieldfrompotential, FTYPE (*prim)[N2M][N3M][NPR], FTYPE (*Upoint)[N2M][N3M][NPR], FTYPE (*Uavg)[N2M][N3M][NPR])
{
  int i,j,k;
  int pl;
  int whichpltoavg[NPR];
...
int get_primitive_centerlocation(int *locpl, int *whichpltoavg, int interporflux, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*U)[N2M][N3M][NPR], FTYPE (*prim_goodlocation)[N2M][N3M][NPR])
{
  int i,j,k,pl;
  ...
  if(1|| avgscheme[1] == WENO5FLAT ||avgscheme[2] == WENO5FLAT ||avgscheme[3] == WENO5FLAT || CONTACTINDICATOR || COMPUTEDRHODP|| SHOCKINDICATOR ) {
	...
	// set guess
	PALLLOOP(pl) prim_goodlocation[i][j][k][pl]=primreal[i][j][k][pl];
	MYFUN(Utoprimgen(0,EVOLVEUTOPRIM, UEVOLVE, U[i][j][k], &geom, prim_goodlocation[i][j][k]),"interpline.c:avg2cen_interp()", "Utoprimgen", 1);
	// if problem with inversion, then reduce to using primreal
	if(pflag[i][j][k][FLAGUTOPRIMFAIL]) PALLLOOP(pl) prim_goodlocation[i][j][k][pl]=primreal[i][j][k][pl];
	...
	myexit( 1 );
      ...
      COMPFULLLOOP  PALLLOOP(pl) prim_goodlocation[i][j][k][pl] = primreal[i][j][k][pl];
...
int get_primitive_fluxlocation(int dir, int interporflux, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*prim_goodlocation)[N2M][N3M][NPR])
{
  int i,j,k,pl;
  ...
  if(1|| avgscheme[dir] == WENO5FLAT || CONTACTINDICATOR || COMPUTEDRHODP|| SHOCKINDICATOR ) {
	...
	COMPLOOPINFP1dir23full PALLLOOP(pl) prim_goodlocation[i][j][k][pl] = 0.5*(primreal[i][j][k][pl]+primreal[im1][j][k][pl]);
      }
      else if(dir==2){
	COMPLOOPINFP1dir13full PALLLOOP(pl) prim_goodlocation[i][j][k][pl] = 0.5*(primreal[i][j][k][pl]+primreal[i][jm1][k][pl]);
      }
      else if(dir==3){
	COMPLOOPINFP1dir12full PALLLOOP(pl) prim_goodlocation[i][j][k][pl] = 0.5*(primreal[i][j][k][pl]+primreal[i][j][km1][pl]);
      }
      else{
...
void flux_interp(int *whichpltoavg, int *ifnotavgthencopy, int whichquantity, int interporflux, int dir, int idel, int jdel, int kdel, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*stencilvar)[N2M][N3M][NPR], FTYPE (*p2interpm)[N2M][N3M][NPR], FTYPE (*p2interpp)[N2M][N3M][NPR], FTYPE (*pleft)[N2M][N3M][NPR], FTYPE (*pright)[N2M][N3M][NPR])
{

  void slope_lim_linetype(int whichquantity, int interporflux, int dir, int idel, int jdel, int kdel, FTYPE (*prims_guess)[N2M][N3M][NPR], FTYPE (*stencilvar)[N2M][N3M][NPR], FTYPE (*p2interpm)[N2M][N3M][NPR],FTYPE (*p2interpp)[N2M][N3M][NPR], FTYPE (*pleft)[N2M][N3M][NPR], FTYPE (*pright)[N2M][N3M][NPR]);
...
void flux_interp_multiple(int *whichpltoavg, int *ifnotavgthencopy, int numdirs, int *whichquantitylist, int *interporfluxlist, int *dirmethodlist, int *Nvec, int *intdirlist, int *fluxdirlist, int *idellist, int *jdellist, int *kdellist, FTYPE (*pr)[N2M][N3M][NPR], FTYPE (*fluxvec[NDIM])[N2M][N3M][NPR])
{
  int i, j, k, pl;
  ...
  if(FLUXDIMENSPLIT==PERFECTUNSPLIT){
	...
	COMPFULLLOOP PLOOP(pl) stencilvar[i][j][k][pl] = fluxvec[fluxdir][i][j][k][pl]; // assumes not flux splitting method since only one input and one output
      }
    ...
    COMPFULLLOOP PLOOP(pl) fluxvec[fluxdir][i][j][k][pl]=0.5*(fluxveca[fluxdir][i][j][k][pl]+fluxvecb[fluxdir][i][j][k][pl]);
...
int avg2cen_interp(int *locpl, int *whichpltoavg, int *ifnotavgthencopy, int whichquantity, int whichavg2cen, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*origin)[N2M][N3M][NPR], FTYPE (*out)[N2M][N3M][NPR])
{
  int get_primitive_centerlocation(int *locpl, int *whichpltoavg, int interporflux, FTYPE (*primreal)[N2M][N3M][NPR], FTYPE (*U)[N2M][N3M][NPR], FTYPE (*prim_goodlocation)[N2M][N3M][NPR]);
  void slope_lim_linetype(int whichquantity, int interporflux, int dir, int idel, int jdel, int kdel, FTYPE (*prim_goodlocation)[N2M][N3M][NPR], FTYPE (*stencilvar)[N2M][N3M][NPR], FTYPE (*p2interpm)[N2M][N3M][NPR],FTYPE (*p2interpp)[N2M][N3M][NPR], FTYPE (*pleft)[N2M][N3M][NPR], FTYPE (*pright)[N2M][N3M][NPR]);
  int dir;
  int idel,jdel,kdel;
  int Nvec[NDIM];
  int i, j, k, pl;
  ...
  if(A2CDIMENSPLIT==PERFECTUNSPLIT){
    ...
    if(numdirs>1){
      ...
      // then need to store original to use for weights
      COMPFULLLOOP PLOOP(pl) stencilvar[i][j][k][pl] = in[i][j][k][pl];
    }
    else stencilvar=NULL; // says use normal input for stencil
	...
	PLOOP(pl) {
	  if(whichpltoavg[pl] || ifnotavgthencopy[pl]){
	    COMPFULLLOOP out[i][j][k][pl] += current_out[i][j][k][pl];
	  }
	}
      ...
      PLOOP(pl){
	if(whichpltoavg[pl] || ifnotavgthencopy[pl]){
	  COMPFULLLOOP out[i][j][k][pl] /= ( (FTYPE)numperms ) ;
	}
      }
...
int addremovefieldfromnpr(int doadd, int *whichpltoavg, int *ifnotavgthencopy, int interptype, int dir, int *nprlocalstart, int *nprlocalend, int *nprlocallist, FTYPE (*in)[N2M][N3M][NPR], FTYPE (*out)[N2M][N3M][NPR])
{
  int pl;
  int pl2,pl3;
  int i,j,k;
  ...
  if(doadd==REMOVEFROMNPR){
    ...
    *nprlocalend=nprend;
	...
	if(whichpltoavg[pl3]==0 && nprlist[pl]==pl3){
	  ...
	  if(ifnotavgthencopy[pl3] && out!=NULL && in!=NULL) COMPFULLLOOP out[i][j][k][pl3]=in[i][j][k][pl3];
	...
	if(nprlist[pl]==pl3){
	  ...
	  if(out!=NULL && in!=NULL) COMPFULLLOOP out[i][j][k][pl3]=in[i][j][k][pl3];
	  for(pl2=pl+1;pl2<=nprend;pl2++) nprlist[pl2-1]=nprlist[pl2]; // moving upper to lower index
	  nprend-=1; // removed dir-field
	  break;
	}
				...
				FTYPE fluxvec_avg[][N2M][N3M][NPR])
{
 ...
 COMPZSLOOP( is, ie, js, je, ks, ke ) {
    ...
    for( index = 0; index <= 1; index++ ) {
      ...
      MYFUN(Utoprimgen(0,EVOLVEUTOPRIM, UEVOLVE, Upoint_updated, &geom, pr_updated),"flux.c:fluxcalc()", "Utoprimgen", 1);
