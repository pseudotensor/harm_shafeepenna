     ...
     //wrapper function that calls a more general conversion function, eno_line_a2c_c2a
{
  ...
  res = eno_line_reconstruct( whichquantity, do_weight_or_recon, stencil_weights_array, CVT_A2C, whichreduce, 
    ...
    shiftit, 
    shockindicator,
    df, dP, monoindicator, P, yin, yout, NULL, NULL ); 
     ...
     //wrapper function that calls a more general conversion function, eno_line_reconstruct
{
  ...
  res = eno_line_reconstruct( whichquantity, do_weight_or_recon, stencil_weights_array, CVT_C2A, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator,  df, dP, monoindicator, P, yin, yout, NULL, NULL ); 
...
int eno_line_c2e( int whichquantity, int dir, int do_weight_or_recon, weno_weights_t *stencil_weights_array, int whichreduce, int preforder, int pl, int bs, int ps, int pf, int bf, int *minorderit, int *maxorderit, int *shiftit, 
		  ...
		  FTYPE *Pindicator, FTYPE *yin, FTYPE *yout_left, FTYPE *yout_right, FTYPE (*youtpolycoef)[NBIGM] ) 
{
  ...
  return(eno_line_reconstruct( whichquantity, do_weight_or_recon, stencil_weights_array, CVT_C2E, whichreduce, preforder, pl, bs, ps, pf, bf, minorderit, maxorderit, shiftit, shockindicator,  df, dP, monoindicator, Pindicator, yin, yout_left, yout_right, youtpolycoef ));
    ...
    if( cvt_type == CVT_C2E && NULL != youtpolycoef ) {
      ...
      for( derorder = 1; derorder <= 4; derorder++ ) {
        ...
        eno_line_reconstruct( whichquantity, RECON_CALC, stencil_weights_array, dertype, whichreduce, 
          ...
          shiftit, 
          shockindicator,
          df, dP, monoindicator, P, yin, youtpolycoef[derorder], NULL, NULL ); 
      }
    }
#endif
...
#define LEFT_WEIGHT (order - 1)
  ...
  for( order = max_order; order > min_order; order-- ) {
    ...
    if( 0.0 < lower_order_fraction ) {   //atch modified to always have two orders
      ...
      create_weights_from_array( cvt_type, lower_order, stencil_weights_array_static[CENTER_POINT].lower_order_weights, &stencil_weights_array_static[CENTER_POINT+1] );
#endif
...
void pass_1d_line_weno_withweights(int whichquantity, int dir, int do_weight_or_recon, weno_weights_t *stencil_weights_array, int recontype, int whichreduce, int preforder, int pl, int bs, int ps, int pe, int be, int *minorder, int *maxorder, int *shift,   FTYPE *shockindicator, FTYPE *stiffindicator, FTYPE *V,  FTYPE *P, FTYPE (*df)[NBIGM], FTYPE (*dP)[NBIGM], FTYPE *etai, FTYPE (*monoindicator)[NBIGM], FTYPE (*yprim)[2][NBIGM], FTYPE (*ystencilvar)[NBIGM], FTYPE (*yin)[NBIGM], FTYPE (*yout)[NBIGM], FTYPE (*youtpolycoef)[NBIGM])
{
  ...
  if(recontype==CVT_C2E){
    ...
    //eno_line_c2e(whichreduce,maxorder,minorder,bs,ps,pe,be,shift,ysend[0], yout[0], yout[1]);  

    //GODMARK: pass maxorder and minorder as arrays for each point of interest
    //		eno_line_c2e( whichquantity, do_weight_or_recon, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift,  shockindicator, df, dP, etai, monoindicator, yprim[UU][0], ysend[0], yout[0], yout[1] );  
    if(PARAMODWENO==0){
      eno_line_c2e( whichquantity, dir, do_weight_or_recon, stencil_weights_array, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift,  shockindicator, stiffindicator, V, P, df, dP, etai, monoindicator, yprim[VSQ][0], ysend[0], yout[0], yout[1], youtpolycoef );
    }
    else{
      paraenohybrid_line_c2e( whichquantity, dir, do_weight_or_recon, stencil_weights_array, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift,  shockindicator, stiffindicator, V, P, df, dP, etai, monoindicator, yprim[VSQ][0], ysend[0], yout[0], yout[1], youtpolycoef );
    }
  }
  else if(recontype==CVT_A2C){
    // yout filled correctly as 1 quantity "deconstructed" to 1 quantity
    //	 eno_line_a2c(whichreduce,maxorder,minorder,bs,ps,pe,be,shift,ysend[0],yout[0]);
    ...
    //      }
#endif
    //		eno_line_a2c( whichquantity, do_weight_or_recon, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, df, dP, monoindicator, yprim[UU][0], ysend[0], yout[0] ); 
    eno_line_a2c( whichquantity, do_weight_or_recon, stencil_weights_array, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, df, dP, monoindicator, yprim[VSQ][0], ysend[0], yout[0] ); 
  }
  ...
  //	eno_line_a2em(1,preforder,bs,ps,pe,be,minorder,maxorder,shift,ysend[0],yout[0]);
  //	eno_line_a2ep(1,preforder,bs,ps,pe,be,minorder,maxorder,shift,ysend[1],yout[1]);
  ...
  //  }
    ...
    // yout filled as each left/right prepared quantity is interpolated as an average to the left/right edges
    //	eno_line_c2a(1,preforder,bs,ps,pe,be,minorder,maxorder,shift,ysend[0],yout[0]);
    // For F1:
    ...
    //
    //////////////////////////////////////
    //		eno_line_c2a( whichquantity, do_weight_or_recon, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, df, dP, monoindicator, yprim[UU][0], ysend[0], yout[0] );   //atch correct
    eno_line_c2a( whichquantity, do_weight_or_recon, stencil_weights_array, whichreduce, preforder, pl, bs, ps, pe, be, minorder, maxorder, shift, shockindicator, df, dP, monoindicator, yprim[VSQ][0], ysend[0], yout[0] );   //atch correct
  }
...
extern void compute_polycoef_line(
  ...
  if(domultipl){
    compute_multipl_weno(MULTIPLTYPE, whichquantity, dir, do_weight_or_recon, recontype, whichreduce, preforder, bs, ps, pe, be, minorder, maxorder, shift,   shockindicator, stiffindicator, Vline,  Pline, df, dP, etai, monoindicator, yprim, ystencilvar, yin, yout, youtpolycoef);
  }
