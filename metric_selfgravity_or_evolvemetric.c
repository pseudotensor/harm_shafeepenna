...
// worry about connection coefficients being accurate enough -- failed to fix dfridr() and apparently eventually broke it.  Compare with prior version
...
// static declarations
static int compute_phi_self_gravity_simple(FTYPE pb[][N2M][N3M][NPR]);
...
// If this function is called, the about to compute new metric
...
//
// 3) static int compute_phi_self_gravity_simple(FTYPE pb[][N2M][N3M][NPR])
//
// 4) static int set_gcov_ks_tov_spcmetric(FTYPE *X, FTYPE *V, FTYPE gcov[][NDIM], FTYPE *gcovpert, SFTYPE *MOrself, SFTYPE *phiself, SFTYPE *vrsqself)
//
// 5) static int set_gcov_bl_tov_spcmetric(FTYPE *X, FTYPE *V, FTYPE gcov[][NDIM], FTYPE *gcovpert, SFTYPE *MOrself, SFTYPE *phiself, SFTYPE *vrsqself)
...
static int get_new_metric_parms(SFTYPE *MBHpar, SFTYPE *apar, SFTYPE *QBHpar)
{
  ...
  if(myid==0){
    ...
    dj = (a0*MBH0+dJ)/pow(MBH0+dE+SMALL,2.0) - a0/(MBH0+SMALL) ;
    // below in infinitesimal form
    //  dj = dJ/(MBH*MBH) - 2.0*j*(dE/MBH);



    // unew holds final averaged conserved quantities from last full step, so just reprocess this after changing grid
    // unew is = $\detg U$ = conserved quantity within cell
...
#if(USEMPI)
  MPI_Bcast(MBHpar,1,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(apar,1,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(QBHpar,1,MPI_SFTYPE,0,MPI_COMM_GRMHD);
#endif
...
int compute_new_metric_longsteps(void)
{
  int dochange, dochangesend;
  int compute_new_metric_and_prims(int whichtime, SFTYPE MBH, SFTYPE a, SFTYPE QBH);
  ...
  if(dochange){
    ...
    compute_new_metric_and_prims(1,MBHpar,apar,QBHpar); // 1 tells us not the initial time
...
int compute_new_metric_and_prims(int whichtime, SFTYPE MBHpar, SFTYPE apar, SFTYPE QBHpar)
{
  SFTYPE dtold;
  extern int set_dt(FTYPE (*prim)[N2M][N3M][NPR], SFTYPE *dt);
  int compute_new_metric_anystep(int whichtime, FTYPE pb[][N2M][N3M][NPR],FTYPE *CUf, FTYPE *Cunew);
  ...
  compute_new_metric_anystep(whichtime, p, &CUf, &Cunew);
...
int compute_new_metric_substep(FTYPE pb[][N2M][N3M][NPR],FTYPE *CUf, FTYPE *Cunew)
{
  int compute_new_metric_anystep(int whichtime, FTYPE pb[][N2M][N3M][NPR],FTYPE *CUf, FTYPE *Cunew);
  ...
  compute_new_metric_anystep(1,pb, CUf, Cunew); // 1 tells us not initial time
...
int store_old_metric(void)
{
  ...
  COMPFULLLOOPP1{
    ...
    for (loc = NPG - 1; loc >= 0; loc--) {
      ...
      DLOOP(jj,kk){
	gcovlast[i][j][k][loc][jj][kk]=gcov[i][j][k][loc][jj][kk];
      }
      DLOOPA(jj){
	gcovpertlast[i][j][k][loc][jj]=gcovpert[i][j][k][loc][jj];
      }

      alphalapselast[i][j][k][loc]=alphalapse[i][j][k][loc];
...
#if(N1>1)
  *dr = fabs(THIRD*(pow(Vip1[1],3.0)-pow(Vim[1],3.0))); // really \delta(r^3/3) = r^2 dr
#else
...
// whether to use unew[] in computing M(r)
...
// can use unew or primitive for substep or skipping step method since unew is kept up-to-date in advance.c relative to where metric is computed
//
...
static int compute_phi_self_gravity_simple(FTYPE pb[][N2M][N3M][NPR])
{
  ...
  int get_rhoref(int i, int j, int k, FTYPE pb[][N2M][N3M][NPR], FTYPE *rhoref);
  FTYPE vrsq;
  static int firsttime=1;
  ...
  COMPZSLOOP(enerpos[X1DN],enerpos[X1UP],enerpos[X2DN],enerpos[X2UP],enerpos[X3DN],enerpos[X3UP]){
    ...
    MYFUN(get_state(pb[i][j][k], &geom, &q) ,"phys.c:source()", "get_state() dir=0", 1);
...
#if(USEMPI)
  MPI_Bcast(&(vrsqvsr_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
#else
  ...
  COMPZSLOOP(enerpos[X1DN],enerpos[X1UP],enerpos[X2DN],enerpos[X2UP],enerpos[X3DN],enerpos[X3UP]){
    ...
    // assumes unew updated for both DOENOFLUX==ENOFINITEVOLUME and (DOENOFLUX==NOENOFLUX)||(DOENOFLUX==ENOFLUXRECON)||(DOENOFLUX==ENOFLUXSPLIT)
    // unew includes \detg factor
    // flipped sign of both UU and RHO terms since energy comes in with negative sign and we want M(r) to be positive


#if(USEUNEW==1)
    get_geometry(i,j,k,CENT,&geom);
    //    alpha = 1.0/sqrt(-geom.gcon[TT][TT]);
    alpha = geom.alphalapse;

    // when EVOLVEMETRICSUBSTEP==0, then unew is final conserved quantity from RK'ing
    // when EVOLVEMETRICSUBSTEP==1, then unew is either final or so-far accumulated conserved quantity.  Not using "Uf" from evolution since Uf represents conserved state at some intermediate state that isn't necessarily going to accumulate to get final RK value.
    // However, non-linearity means that using unew each time is more correct with RK method so should converge to fourth order in correct way
    dM = -unew[i][j][k][UU]*dVF/(alpha*(q.ucov[TT]));

    if(REMOVERESTMASSFROMUU==2){
      // then need to add back in the rest-mass
      dM += unew[i][j][k][RHO]*dVF/(alpha);
    }
    else{
      // then already correct
    }

    dJ = unew[i][j][k][U3]*dVF/(alpha*(q.ucov[TT]));
    ...
    MYFUN(get_state(pb[i][j][k], &geom, &q) ,"phys.c:source()", "get_state() dir=0", 1);
    DLOOPA(jj) mhd_calc_0(pb[i][j][k], jj, &geom, &q, mhdprime[jj]); // includes rest-mass
    ...
    dM = (
	  ...
	  unew[i][j][k][RHO]*dVF*(1.0+1.0/uu0ud0)
	  );
    
    
    dJ = (
	  jacvol*rho*(ucovnative[U3])*(-1.0/uu0ud0)+
	  unew[i][j][k][U3]*dVF*(1.0+1.0/uu0ud0)
	  );
    ...
    */
...
#if(0)
    // can't use unew[UU] since it had added to it the gravitational changes
    unewuueff = unew[i][j][k][UU]*dVF;
    if(REMOVERESTMASSFROMUU==2){
      // then need to add back in the rest-mass
      unewuueff -= unew[i][j][k][RHO]*dVF;
    }
    // effective rho u^t from MHD stress-tensor with gdet in it
    rhouu0eff = unewuueff/(q.ucov[TT]);
#else
    unewuueff = mhdprime[TT][TT]*(geom.g)*dVF;
    rhouu0eff = unewuueff/(q.ucov[TT]);
#endif
...
#if(0) // DEBUG
    if((jacvol-(geom.g)*dVF)/(jacvol+SMALL)>1.2){
    ...
    //    Jeff = (mhdprime[TT][PH]*(geom.g))/(ucovnative[TT]);
    //
    // unlike energy, angular momentum is conserved
    // still use mhd instead of unew since unew might eventually have sources
    Jeff = (mhdprime[TT][PH]*(geom.g))*dVF;
    ...
    //    PLOOP(pl) dualfprintf(fail_file,"pb[%d]=%21.15g\n",pl,pb[i][j][k][pl]);
...
#define GAMMAGRAV(MOr) (1.0/(1.0-2.0*(MOr)+vrsqvsr_tot[ii])) // relativistic TOV factor
	...
	whereformedblackhole=jj;
      ...
      // DEBUG
      //phivsr_tot[ii] = 0.0;
      //phivsr_tot[ii] = 1.0/pow(absrcent,1.0);
...
#define GAMMAGRAVTEST(MOr) (1.0/(1.0-2.0*(MOr))) // relativistic TOV factor
  ...
  ///////////////////////////////////////
#if(USEMPI)
  MPI_Bcast(&(Mvsr_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(Mvsrface1_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(Jvsr_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(Jvsrface1_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(MOrvsr_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  MPI_Bcast(&(phivsr_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);
  //  MPI_Bcast(&(vrsqvsr_tot[-N1BND]),NUMGRAVPOS,MPI_SFTYPE,0,MPI_COMM_GRMHD);

  MPI_Bcast(&formedblackhole,1,MPI_INT,0,MPI_COMM_GRMHD);
  MPI_Bcast(&whereformedblackhole,1,MPI_INT,0,MPI_COMM_GRMHD);
...
//  rhoref=0.0;
    ...
    //  GRAVLOOP(ii) phivsr_tot[ii]=-0.05/pow(rcent_tot[ii],1.0);
...
int get_rhoref(int i, int j, int k, FTYPE pb[][N2M][N3M][NPR], FTYPE *rhoref)
{
  ...
  MYFUN(get_state(pb[i][j][k], &geom, &q) ,"phys.c:source()", "get_state() dir=0", 1);
  DLOOPA(jj) mhd_calc_0(pb[i][j][k], jj, &geom, &q, mhd[jj]); // includes rest-mass
...
int bound_spacetime_inside_horizon(void)
{
  ...
  if(BCtype[X1DN]==R0SING){ // global condition that all CPUs know about
    ...
    if(globali<startpos[1]-N1BND) locali=-100; // indicates not acting on this CPU
    ...
    LOOPF3 LOOPF2 LOOPF1{// SECTIONMARK: Should this be FULL or COMPFULL?
	...
	if(startpos[1]+i < globali){
	  ...
	  DLOOP(jj,kk) DLOOPA(ll){
	    //	    conn[i][j][k][jj][kk][ll] = conn[ri][rj][rk][jj][kk][ll];
	    // decided to make space-time effectively flat within fake boundary region
	    conn[i][j][k][jj][kk][ll] = 0.0;
	  }

	  DLOOPA(jj){
	    //conn2[i][j][k][jj] = conn2[ri][rj][rk][jj];
	    // decided to make space-time effectively flat within fake boundary region
	    conn2[i][j][k][jj] = 0.0;
	  }

	  DLOOP(jj,kk) for (loc = NPG - 1; loc >= 0; loc--) {
	    gcon[i][j][k][loc][jj][kk] = gcon[ri][rj][rk][loc][jj][kk];
	    // setting last to be equal to current do dg/dt = 0 in this region
	    gcov[i][j][k][loc][jj][kk] = gcov[ri][rj][rk][loc][jj][kk];
#if(DOEVOLVEMETRIC)
	    gcovlast[i][j][k][loc][jj][kk] = gcov[i][j][k][loc][jj][kk] ;
#endif
	  }

	  DLOOPA(jj) for (loc = NPG - 1; loc >= 0; loc--) {
	    // setting last to be equal to current do dg/dt = 0 in this region
	    gcovpert[i][j][k][loc][jj] = gcovpert[ri][rj][rk][loc][jj];
#if(DOEVOLVEMETRIC)
	    gcovpertlast[i][j][k][loc][jj] = gcovpert[i][j][k][loc][jj] ;
#endif
	  }

	  alphalapse[i][j][k][loc] = alphalapse[i][j][k][loc] ;
#if(DOEVOLVEMETRIC)
	  alphalapselast[i][j][k][loc] = alphalapse[i][j][k][loc] ;
#endif

	  for (loc = NPG - 1; loc >= 0; loc--) {
	    gdet[i][j][k][loc] = gdet[ri][rj][rk][loc];
	    eomfunc[i][j][k][loc] = eomfunc[ri][rj][rk][loc];
#if(GDETVOLDIFF)
	    gdetvol[i][j][k][loc] = gdetvol[ri][rj][rk][loc];
#endif
	  }

#if(VOLUMEDIFF)
	  DLOOPA(jj) idxvol[i][j][k][jj] = idxvol[ri][rj][rk][jj];
#endif
...
int set_gcov_ks_tov_spcmetric(FTYPE *X, FTYPE *V, FTYPE gcov[][NDIM], FTYPE *gcovpert, SFTYPE *MOrself, SFTYPE *phiself, SFTYPE *vrsqself)
{
  int jj,kk;
  ...
  *vrsqself=0.0;
  ...
  // 0-out metric
  DLOOP(jj,kk) gcov[jj][kk]=0.0;

  gcov[TT][TT] = -exp(2.0*phiself[0]);
  genpot=vrsqself[0] - 2.0*MOrself[0];
  gcov[RR][RR] = 1.0+fabs(-genpot); // [RR][RR] term is positive and be 1 in limit that genpot=0
  // [RR][TT] term must carry sign of r
  gcov[RR][TT] = gcov[TT][RR] = sqrt(-gcov[TT][TT]/(1.0-fabs(genpot)))*(-genpot);
  gcov[TH][TH] = r2;
  gcov[PH][PH] = r2*sh2;


  gcovpert[TT] = gcov[TT][TT]+1.0; // no obvious way to decouple non-linear piece
...
int set_gcov_bl_tov_spcmetric(FTYPE *X, FTYPE *V, FTYPE gcov[][NDIM], FTYPE *gcovpert, SFTYPE *MOrself, SFTYPE *phiself, SFTYPE *vrsqself)
{
  int jj,kk;
  FTYPE r,r2,th,sh,sh2;
  FTYPE genpot;
  ...
  *vrsqself=0.0;
  ...
  gcov[TT][TT] = -exp(2.0*phiself[0]);
  genpot=vrsqself[0] - 2.0*MOrself[0];
  gcov[RR][RR] = 1.0/(1.0+fabs(genpot));
  gcov[RR][TT] = gcov[TT][RR] = 0.0;
  gcov[TH][TH] = r2;
  gcov[PH][PH] = r2*sh2;


  gcovpert[TT] = gcov[TT][TT]+1.0; // no obvious way to decouple non-linear piece
  gcovpert[RR] = gcov[RR][RR]-1.0; // no obvious way ...
  gcovpert[TH] = gcov[TH][TH]-1.0;
  gcovpert[PH] = gcov[PH][PH]-1.0;
