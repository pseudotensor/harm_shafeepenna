...
int flux_compute_general(int i, int j, int k, int dir, struct of_geom *ptrgeom, FTYPE CUf, FTYPE *p_c, FTYPE *p_l, FTYPE *p_r, FTYPE *F, FTYPE *ctopptr)
{
  ...
  //WRONG  MYFUN(p2SFUevolve(dir, p_c, ptrgeom, &state_c, F_c, U_c),"step_ch.c:fluxcalc()", "p2SFUevolve()", 2);
  //  deconvolve_flux(dir, ptrgeom, p_c, p_l, p_r, &state_c, &state_l, &state_r, F_c, F_l, F_r, U_c, U_l, U_r);
#endif
...
#if(MERGEDC2EA2CMETHOD==1)
  // WRONG:
  //  MYFUN(p2SFUevolve(dir, p_c, ptrgeom, &state_c, FEM_c, UEM_c),"step_ch.c:fluxcalc()", "p2SFUevolve()", 2);
  //  deconvolve_flux_ma(dir, ptrgeom, p_c, p_l, p_r, &state_c, &state_l, &state_r, F_c, F_l, F_r, U_c, U_l, U_r);
  //  deconvolve_flux_em(dir, ptrgeom, p_c, p_l, p_r, &state_c, &state_l, &state_r, FEM_c, FEM_l, FEM_r, UEM_c, UEM_l, UEM_r);
#endif
...
#if(FLUXDUMP)
  PLOOP(pl) fluxdump[i][j][k][4*NPR + (dir-1)*NPR*5 + NPR*1 + pl]=F_l[pl];
  PLOOP(pl) fluxdump[i][j][k][4*NPR + (dir-1)*NPR*5 + NPR*2 + pl]=F_r[pl];
  PLOOP(pl) fluxdump[i][j][k][4*NPR + (dir-1)*NPR*5 + NPR*3 + pl]=p_l[pl];
  PLOOP(pl) fluxdump[i][j][k][4*NPR + (dir-1)*NPR*5 + NPR*4 + pl]=p_r[pl];
...
int forceflux_compute(int dir,struct of_geom *geom, FTYPE cmin, FTYPE cmax, FTYPE ctop, FTYPE cforce, FTYPE *p_l, FTYPE *p_r, FTYPE *U_l, FTYPE *U_r,FTYPE *F_l,FTYPE *F_r, FTYPE *F)
{
  ...
  MYFUN(Utoprimgen(0,EVOLVEUTOPRIM,UEVOLVE, umid, geom, pmid),"flux.c:flux_compute()", "Utoprimgen", 1);
  doforceflux=1;
...
#define MUSTACOEFTYPE 1
    ...
    MYFUN(Utoprimgen(0,EVOLVEUTOPRIM,UEVOLVE, U_l, geom, p_l),"flux.c:mustaflux_compute()", "Utoprimgen", 1);
    if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
    ...
    MYFUN(Utoprimgen(0,EVOLVEUTOPRIM,UEVOLVE, U_r, geom, p_r),"flux.c:mustaflux_compute()", "Utoprimgen", 1);
    if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
  ...
  for(mustaloop=0;mustaloop<NUMMULTIMUSTAITERS;mustaloop++){
    ...
    */
    ...
    for(mustacellloop=1;mustacellloop<=2*NUMLOCALCELLS;mustacellloop++){
      ...
      MYFUN(Utoprimgen(0,EVOLVEUTOPRIM,UEVOLVE, Unow, geom, pnow),"flux.c:mustaflux_compute()", "Utoprimgen", 1);
      if(pflag[geom->i][geom->j][geom->k][FLAGUTOPRIMFAIL]){
	if(debugfail>=1) dualfprintf(fail_file,"Failed to find inversion for MUSTAFORCEFLUX(right): nstep=%ld t=%21.15g i=%d j=%d k=%d\n",nstep,t,geom->i,geom->j,geom->k);
	domustaflux=0;
	break;
