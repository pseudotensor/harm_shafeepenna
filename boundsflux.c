...
int bound_flux_user(int boundstage, SFTYPE boundtime, int boundvartype, FTYPE F1[][N2M][N3M][NPR], FTYPE F2[][N2M][N3M][NPR], FTYPE F3[][N2M][N3M][NPR])
{
  int i, j, k, pl;
  ...
  if ( (mycpupos[1] == 0)&&(mycpupos[1] == ncpux1 - 1) ) {
	...
	rj=j;
	rk=k;
	LOOPBOUND1IN PBOUNDLOOP(pl) F1[i][j][k][pl] = F1[ri+1+i][rj][rk][pl]; // for i=0 -> i=N1
	if(FLUXB==FLUXCTSTAG){
	  if(N2>1) LOOPBOUND1IN PBOUNDLOOP(pl) F2[i][j][k][pl] = F2[ri+1+i][rj][rk][pl];
	  if(N3>1) LOOPBOUND1IN PBOUNDLOOP(pl) F3[i][j][k][pl] = F3[ri+1+i][rj][rk][pl];
	}
	...
	rj=j;
	rk=k;
	LOOPBOUND1OUT PBOUNDLOOP(pl) F1[i][j][k][pl] = F1[ri+(i-N1)][rj][rk][pl]; // for i=N1 -> i=0
	if(FLUXB==FLUXCTSTAG){
	  if(N2>1) LOOPBOUND1OUT PBOUNDLOOP(pl) F2[i][j][k][pl] = F2[ri+(i-N1)][rj][rk][pl]; // for i=N1 -> i=0
	  if(N3>1) LOOPBOUND1OUT PBOUNDLOOP(pl) F3[i][j][k][pl] = F3[ri+(i-N1)][rj][rk][pl]; // for i=N1 -> i=0
	}
      }
    }
  }
  ...
  if ( (mycpupos[2] == 0)&&(mycpupos[2] == ncpux2 - 1) ) {
	...
	rj=rjout;
	rk=k;
	LOOPBOUND2IN PBOUNDLOOP(pl) F2[i][j][k][pl] = F2[ri][rj+1+j][rk][pl]; // for j=0 -> j=N2
	if(FLUXB==FLUXCTSTAG){
	  if(N1>1) LOOPBOUND2IN PBOUNDLOOP(pl) F1[i][j][k][pl] = F1[ri][rj+1+j][rk][pl]; // for j=0 -> j=N2
	  if(N3>1) LOOPBOUND2IN PBOUNDLOOP(pl) F3[i][j][k][pl] = F3[ri][rj+1+j][rk][pl]; // for j=0 -> j=N2
	}
	...
	rj=rjin;
	rk=k;
	LOOPBOUND2OUT PBOUNDLOOP(pl) F2[i][j][k][pl] = F2[ri][rj+(j-N2)][rk][pl]; // for j=N2 -> j=0
	if(FLUXB==FLUXCTSTAG){
	  if(N1>1) LOOPBOUND2OUT PBOUNDLOOP(pl) F1[i][j][k][pl] = F1[ri][rj+(j-N2)][rk][pl]; // for j=N2 -> j=0
	  if(N3>1) LOOPBOUND2OUT PBOUNDLOOP(pl) F3[i][j][k][pl] = F3[ri][rj+(j-N2)][rk][pl]; // for j=N2 -> j=0
	}
      }
    }
  }
  ...
  if ( (mycpupos[3] == 0)&&(mycpupos[3] == ncpux3 - 1) ) {
	...
	rj=j;
	rk=rkout;
	LOOPBOUND3IN PBOUNDLOOP(pl) F3[i][j][k][pl] = F3[ri][rj][rk+1+k][pl]; // for k=0 -> k=N3
	if(FLUXB==FLUXCTSTAG){
	  if(N1>1) LOOPBOUND3IN PBOUNDLOOP(pl) F1[i][j][k][pl] = F1[ri][rj][rk+1+k][pl]; // for k=0 -> k=N3
	  if(N2>1) LOOPBOUND3IN PBOUNDLOOP(pl) F2[i][j][k][pl] = F2[ri][rj][rk+1+k][pl]; // for k=0 -> k=N3
	}
	...
	rj=j;
	rk=rkin;
	LOOPBOUND3OUT PBOUNDLOOP(pl) F3[i][j][k][pl] = F3[ri][rj][rk+(k-N3)][pl]; // for k=N3 -> k=0
	if(FLUXB==FLUXCTSTAG){
	  if(N1>1) LOOPBOUND3OUT PBOUNDLOOP(pl) F1[i][j][k][pl] = F1[ri][rj][rk+(k-N3)][pl]; // for k=N3 -> k=0
	  if(N2>1) LOOPBOUND3OUT PBOUNDLOOP(pl) F2[i][j][k][pl] = F2[ri][rj][rk+(k-N3)][pl]; // for k=N3 -> k=0
	}
      }
    }
  }
  ...
  if (mycpupos[2] == 0) {
	...
	rj=rjin;
	rk=k;
	LOOPBOUND2IN PBOUNDLOOP(pl)  F2[i][j][k][pl] = F2[ri][rj+(rj-j)][rk][pl]; // self-assigns for j=0
      }
    }
  ...
  if (mycpupos[2] == ncpux2-1) {
	...
	rj=rjout;
	rk=k;
	LOOPBOUND2OUT PBOUNDLOOP(pl)  F2[i][j][k][pl] = F2[ri][rj+(rj-j+2)][rk][pl]; // self-assigns for j=N
      }
    }
