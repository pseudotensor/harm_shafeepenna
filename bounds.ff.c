...
int bound_prim_user(int boundstage, FTYPE prim[][N2M][N3M][NPR])
{
  ...
  //
  void x1_star(FTYPE (*prim)[N2M][N3M][NPR]);
  void x1_outer(FTYPE (*prim)[N2M][N3M][NPR]);
  void x2_inner_polar(FTYPE (*prim)[N2M][N3M][NPR]);
  void x2_outer_polar(FTYPE (*prim)[N2M][N3M][NPR]);
  void x2_disk(int dir, FTYPE (*prim)[N2M][N3M][NPR]);
...
void x1_star(FTYPE (*prim)[N2M][N3M][NPR])
{
  ...
  void bound_face1dn(int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface);
  ...
  if( mycpupos[1] == 0 ){ 
	...
	if(DISKDIR==X2DN && (startpos[2]+j==totalsize[2]-1 || startpos[2]+j==totalsize[2]-2 )){
	  ...
	  // outflow \detg B^2
	  pl = B2; prim[i][j][k][pl] = prim[ri][rj][rk][pl]*(rgeom.g)/(geom.g);
	}

	// outflow B_\phi
        pl = B3; prim[i][j][k][pl] = prim[ri][rj][rk][pl]*(rgeom.gcov[3][3])/(geom.gcov[3][3]);

        //reset velocities to stationary ones
	set_vel_stataxi(&geom,pother[OMEGAFCENT][i][j][k],pother[VPARCENT][i][j][k],prim[i][j][k]);
...
//saves the face values in pother[xxxFACE1][i][j][k]
void bound_face1dn(int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface)
{
  void outflow_face1dn(int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface);
  ...
  PLOOP(pl) pother[RHOFACE1+pl][i][j][k] = prface[pl];
}
...
void set_face1dn(int i, int j, int k, FTYPE *prface)
{
  ...
  if(is_on_surface(X1DN,i,j,k,FACE1)){
    ...
    prface[B1] = pother[B1FACE1][i][j][k];

    //reset velocities to stationary ones (VPAR analytical)
    set_vel_stataxi(&geomf,pother[OMEGAFFACE1][i][j][k],pother[VPARFACE1][i][j][k],prface);
  }
}
...
void outflow_face1dn(int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface)
{
  ...
  if(is_on_surface(X1DN,i,j,k,FACE1)){
    ...
    // outflow as copy
    //    if(sign(rprim[B3])==sign(-prface[B1]*pother[OMEGAFFACE1][i][j][k])){
    ...
    prface[B3] = rprim[B3]*(geomc.gcov[3][3])/(geomf.gcov[3][3]);
  }
}
...
void set_plpr(int dir, int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *p_l, FTYPE *p_r)
{
  ...
  PLOOP(pl) prface[pl] = 0.5 * ( p_l[pl] + p_r[pl] );  //initialize prface with boundary values
  ...
  if(is_on_surface(X1DN,i,j,k,FACE1) && dir == 1){
    ...
    PLOOP(pl){
      ...
      PLOOP(pl) prface[pl] = pother[RHOFACE1+pl][i][j][k]; // pl=0..NPR-1  (assumes ordering of pother[RHOFACE1->B3FACE1] as for standard primitives)
#endif
...
#elif( WHICHSETFACE == BCSETFACE )
    PLOOP(pl) prface[pl] = pother[RHOFACE2+pl][i][j][k];  // pl=0..NPR-1  (assumes ordering of pother[RHOFACE1->B3FACE1] as for standard primitives)
#endif
...
#define TSTATIC (0.0)
  ...
  int set_vel_stataxi(struct of_geom *geom, FTYPE omegaf, FTYPE vpar, FTYPE *pr);
  void bound_face2disk(int dir, int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface);
  int i,j,k;
  int ri,rj,rk;
  int pl;
  FTYPE X[NDIM], V[NDIM];
  ...
  if(dobounds) {
	...
	// set densities analytically
        pl = RHO; prim[i][j][k][pl] = panalytic[i][j][k][pl];
        pl = UU; prim[i][j][k][pl] = panalytic[i][j][k][pl];

	// outflow \detg B^1
        pl = B1; prim[i][j][k][pl] = prim[ri][rj][rk][pl]*(rgeom.g)/(geom.g);

        // set the normal magnetic field to the initial value
        pl = B2; prim[i][j][k][pl] = panalytic[i][j][k][pl];
...
//
///////////////////////////
void outflow_face2disk(int dir, int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface)
{
  int pl;
  FTYPE Bcon[NDIM];
  ...
  if(is_on_surface(dir,i,j,k,FACE2)){
    ...
    //first, outflow everything
    PLOOP(pl) prface[pl] = prim[ri][rj][rk][pl];
    ...
    // outflow B1
    prface[B1]=prim[ri][rj][rk][B1]*(geomc.g)/(geomf.g);

    // outflow B_\phi
    prface[B3]=prim[ri][rj][rk][B3]*fabs(geomc.gcov[3][3])/(geomf.gcov[3][3]);
...
void set_face2(int dir, int i, int j, int k, FTYPE *prface)
{
  ...
  if(is_on_surface(dir,i,j,k,FACE2) && dir == DISKDIR){
    ...
    // densities
    prface[RHO] = pother[RHOFACE2][i][j][k];
    prface[UU] = pother[UUFACE2][i][j][k];

    // set B2
    prface[B2]=pother[B2FACE2][i][j][k];

    //get velocity
    set_vel_stataxi(&geomf,pother[OMEGAFFACE2][i][j][k],pother[VPARFACE2][i][j][k],prface);
...
void bound_face2disk(int dir, int i, int j, int k, FTYPE (*prim)[N2M][N3M][NPR], FTYPE *prface)
{
  ...
  PLOOP(pl){
    pother[RHOFACE2+pl][i][j][k]=prface[pl];
  }
}
...
void x1_outer(FTYPE (*prim)[N2M][N3M][NPR])
{
  ...
  if( mycpupos[1] == ncpux1-1 ) {
	...
	// outflow B^1
        pl = B1; prim[i][j][k][pl] = prim[ri][rj][rk][pl];
	...
	// outflow velocities (copy)
        pl = U1; prim[i][j][k][pl] = prim[ri][rj][rk][pl];
        pl = U2; prim[i][j][k][pl] = prim[ri][rj][rk][pl];
        pl = U3; prim[i][j][k][pl] = prim[ri][rj][rk][pl];
             ...
             FTYPE *p2interp_l, FTYPE *p2interp_r )
{
  ...
  PLOOP(pl) {
    ...
    if( (locallim<PARA) && (LIMADJUST==0) ) {
      ...
      if( reconavoidbcdn[dir] == 1 && ijkabs == 0 ) {
        dq[i][j][k][pl] = dq[i + idel][j + jdel][k + kdel][pl];
      }
      //upper boundary
  ...
  if(dir==2){
    if(ijkabs==totalsize[dir]-2){
      dq[i][j][k][B2]=(0.0-p2interp[i][j][k][B2])/(1.5);
      dq[i][j][k][U2]=(0.0-p2interp[i][j][k][U2])/(1.5);
    }
    else if(ijkabs==totalsize[dir]-1){
      dq[i][j][k][B2]=(0.0-p2interp[i][j][k][B2])/(0.5);
      dq[i][j][k][U2]=(0.0-p2interp[i][j][k][U2])/(0.5);
    }
  }
